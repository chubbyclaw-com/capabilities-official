...
| `version`     | string | 否   | Semver，如 `"1.1.0"`。UI 中显示的权威 Capability 版本；省略则默认 `1.0.0` |
| `author`      | object | 否   | `{ "name": "...", "email": "...", "url": "..." }`                         |
| `credentials` | array  | 否   | 此 Capability 需要的凭据声明（见 [`credentials`](#credentials)）          |

### `credentials`

声明 Capability 运行所需的 API Key 或 OAuth 连接。没有 `credentials` 字段的 Capability 退化到原有的自由编辑 env override 模式。
...
"credentials": [
  ...
  },
  {
    ...
    "description": "访问你的 Google Drive 文件",
    "required": true
  }
]
```

`credentials` 数组中每个条目的字段：

| 字段           | 类型        | 适用类型     | 必填        | 说明                                                                                           |
| -------------- | ----------- | ------------ | ----------- | ---------------------------------------------------------------------------------------------- |
| `name`         | string      | 两种         | 是          | `api_key` 类型：环境变量名（如 `TAVILY_API_KEY`）；`oauth` 类型：连接标识（如 `GOOGLE_DRIVE`） |
| `type`         | string      | 两种         | 是          | `"api_key"` 或 `"oauth"`                                                                       |
| `provider`     | string      | 仅 `oauth`   | 是（oauth） | `"google"`、`"github"`、`"slack"` 或 `"custom"`                                                |
| `scopes`       | string 数组 | 仅 `oauth`   | 否          | 请求的 OAuth scopes（如 `["drive.readonly"]`）                                                 |
| `required`     | boolean     | 两种         | 否          | 是否必须提供此凭据才能运行，默认 `true`                                                        |
| `description`  | string      | 两种         | 否          | 凭据选择器中展示的用户可读描述                                                                 |
| `help_url`     | string      | 仅 `api_key` | 否          | 获取 API Key 的链接                                                                            |
| `oauth_config` | object      | 仅 `oauth`   | 否          | 自定义 OAuth 端点，仅在 `provider` 为 `"custom"` 时使用                                        |

#### 自定义 OAuth Provider（`oauth_config`）

当 `provider` 为 `"custom"` 时，在 `oauth_config` 中提供 OAuth 端点：
...
用户通过 env override 提供 `client_id` / `client_secret`。
